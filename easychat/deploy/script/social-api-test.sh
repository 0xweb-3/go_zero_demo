#!/bin/bash
reso_addr='crpi-8mkzdazjkdk1gs4d.cn-hangzhou.personal.cr.aliyuncs.com/xin_go_zero/social-api-dev'
tag='latest'
...
docker run -p 9001:7001 -e POD_IP=${pod_ip} --name=${container_name} -d ${reso_addr}:${tag}